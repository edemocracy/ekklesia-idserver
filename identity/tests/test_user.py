...
def test_send(request,accounts,tokens,mails,bilateral,client,defect):
    ...
    response, out = api(client,'user/mails/','post',data,token=token)
    assert response.status_code == 200
    data = {'identity':'portal','content':{'subject':'hallo3','body':'foo'},'sign':True,'encrypt':True}
    response, out = api(client,'user/mails/','post',data,token=token)
    assert response.status_code == 200
    ...
    for mail, flags in imaprecv:
        ...
        else: assert result['encrypted']==(subj=='hallo3') and result['signed'] and result['fingerprints']==[key1]
        #print mail
