...
from accounts.models import Account, NestedGroup
...
class Share(models.Model):
    ...
    def next_version(self):
        with transaction.atomic():
            share = Share.objects.select_for_update().get(id=self.id)
            share.version += 1
...
class ShareObject(models.Model):
    ...
    def __unicode__(self): return "%s/%i" % (self.share.name,self.no)
...
class ShareChange(models.Model):
    ...
    def __unicode__(self): return "%s/%i:%i" % (self.share.name,self.no,self.version)
...
class UserList(models.Model):
    ...
    def next_version(self):
        with transaction.atomic():
            ulist = UserList.objects.select_for_update().get(id=self.id)
            ulist.version += 1
    ...
    def is_member(self,user):
        return self.members.filter(member=user.id).exists()
